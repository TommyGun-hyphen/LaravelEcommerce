    ...
    <div class="overflow-x-auto w-2/3 mx-auto">
      ...
      <table class="border-3 border-indigo-300 rounded table table-zebra w-full">
        ...
        <tbody>
            ...
            <tr>
              <form action="/admin/subcategory/{{ $category->id }}" method="POST">
                @csrf
                @method('PUT')
                <th>{{ $subcategory->id }}</th>
                <td><input type="text" name="name" value="{{ $subcategory->name }}" class="input input-bordered w-full max-w-xs"></td>

                <td>{{ $subcategory->slug }}</td>
                <td><a class="btn" href="/admin/subcategory/{{ $subcategory->slug }}">produits</a></td>
                <td><div class="card-actions justify-center">
                  <button type="submit" class="btn btn btn-circle btn-neutral btn-sm"><i class="fa-solid fa-wrench"></i></button>
                </td>
              </form>
