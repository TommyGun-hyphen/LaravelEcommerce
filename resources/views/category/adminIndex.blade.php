            ...
            @foreach($categories as $category)
                ...
                <tr>
                    ...
                    <form action="/admin/category/{{ $category->id }}" method="POST">
                        ...
                        @csrf
                        <th>{{ $category->id }}</th>
                        <td><input type="text" name="name" value="{{ $category->name }}" class="input input-bordered w-full max-w-xs"></td>
                        <td>{{ $category->slug }}</td>
                        <td><a href="/admin/category/{{ $category->slug }}" class="btn">sous categories</a></td>
                        
                            <td><div class="card-actions justify-center">
                                    <button type="submit" class="btn btn btn-circle btn-neutral btn-sm"><i class="fa-solid fa-wrench"></i></button></td>
                        </form>
                    <form action="/admin/category/{{ $category->id }}" method="POST">
                        @method('delete')
                        @csrf
