...
<div class="container my-5">
    <div class="grid grid-cols-1 lg:grid-cols-2 ">
        ...
        <div class="my-4">
            <div style="background-color: hsl(var(--s))" class="badge badge-outline text-white float-right mx-3">{{ ($product->is_used == 1)?'utilisé':'neuf' }}</div>
            <h2 class="h2 uppercase">{{ $product->name }}</h2>
            <h4 class="h4 font-bold text-zinc-900">
                {{ $product->price }} DH
            </h4>
            <h5 class="line-through h5">
                {{ ($product->price_old)? $product->price_old .' DH' :'' }}
            </h5>
            ...
            <div class="flex justify-center">
                @if (!$product->in_stock)
                    <div class="badge badge-outline border-red-800 bg-red-600 text-white mx-auto mt-2">Rupture de stock</div>
                @endif
            </div>
            <div class="flex align-items-center justify-around">
                <div class="custom-number-input h-10 w-32  mx-3">
