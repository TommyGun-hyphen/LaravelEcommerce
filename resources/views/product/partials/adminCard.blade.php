<div class="hover:shadow-2xl card mx-auto w-64 bg-base-100 shadow-xl">
    ...
    <div class="card-body">
        <a  href="/admin/product/{{ $product->slug }}" class="card-title">
            {{ $product->name }}
            <div>
                <div class="badge badge-secondary">{{ $product->price }} DH</div>
                <span class="text-sm line-through">{{ ($product->price_old) ? $product->price_old.' DH' : ''}}</span>
            </div>
        </a>
        <div class="card-actions justify-end">
            <div class="badge badge-outline">{{ $product->subCategory()->first()->name }} > {{ $product->subCategory()->first()->category()->first()->name }}</div>
        </div>
